 ...
 * @since 0.2
 */
public class MaxMF<T extends Number & Comparable<T>> implements BiFunction<Collection<T>, MembershipFunction<T>, Map<Double, Double>> {
	 ...
	 */
	public Map<Double, Double> apply(Collection<T> x,
            MembershipFunction<T> mf) {
        Map<Double, Double> max = new HashMap<Double, Double>();
        BigDecimal maxValue = BigDecimal.valueOf(0.0);
        boolean first = true;
        for(T value : x) {
            BigDecimal temp = new BigDecimal(mf.apply(value), new MathContext(precision, roundingMode));
    ...
    public static <T extends Number & Comparable<T>> Map<Double, Double> of(Collection<T> col,
            MembershipFunction<T> mf) {
        return new MaxMF<T>().apply(col, mf);
    }
